...
export default function Pagination({ pageSize, queryConfig }: Props) {
  ...
  const renderPagination = () => {
    ...
    return Array(pageSize)
      ...
      .map((_, index) => {
        ...
        } else if (pageNumber < page - RANGE && page >= pageSize - RANGE * 2 && pageNumber > RANGE) {
          return renderDotBefore(index)
        }
  ...
  return (
    <div className='flex flex-wrap mt-6 justify-center'>
      {page === 1 ? (
        <button className='bg-white/60 rounded  px-3 py-2 shadow-sm mx-2  border cursor-not-allowed border'>
          Prev
        </button>
      ) : (
        <Link
